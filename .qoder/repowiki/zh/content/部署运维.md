# 部署运维

<cite>
**本文引用的文件列表**
- [main.go](file://main.go)
- [install.sh](file://scripts/install.sh)
- [gen_cert.sh](file://scripts/gen_cert.sh)
- [README.md](file://README.md)
- [ipv6-proxy.service](file://systemd/ipv6-proxy.service)
- [ndppd.conf.example](file://systemd/ndppd.conf.example)
- [config.yaml.example](file://config.yaml.example)
- [go.mod](file://go.mod)
</cite>

## 更新摘要
**变更内容**
- 新增详细日志系统支持，包含INFO、WARN、ERROR、DEBUG四个级别
- 添加调试模式开关 `-v` 参数，用于启用详细调试日志输出
- 增强错误报告和故障排查能力，提供更详细的错误信息
- 修正文件引用路径，从 REDME.md 更正为 README.md

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与容量规划](#性能与容量规划)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：常用命令与最佳实践](#附录常用命令与最佳实践)

## 简介
本指南面向生产环境，提供 IPv6 代理池在不同环境（物理服务器、虚拟机、容器）下的部署方案，覆盖自动化脚本使用、systemd 服务管理、日志与监控告警、安全加固、性能调优与故障恢复等。项目为纯 Go 实现，支持 HTTP CONNECT 与 SOCKS5 双协议，强制通过 IPv6 出口，内置并发限流，适用于与 v2ray/xray 配合的场景。

**更新** 新增完善的日志系统和调试模式，显著提升故障排查能力和运维效率。

## 项目结构
仓库包含可执行程序源码、一键安装脚本、证书生成脚本、示例配置与服务单元文件等。整体组织清晰，便于在多种环境中快速落地。

```mermaid
graph TB
A["main.go"] --> B["HTTP CONNECT 处理"]
A --> C["SOCKS5 处理"]
A --> D["多级日志系统<br/>INFO/WARN/ERROR/DEBUG"]
A --> E["调试模式 -v 开关"]
D --> F["结构化错误报告"]
G["scripts/install.sh"] --> H["系统参数优化<br/>内核参数写入 sysctl.d"]
G --> I["编译并安装二进制到 /opt/ipv6-proxy"]
G --> J["复制 systemd 服务单元"]
K["scripts/gen_cert.sh"] --> L["生成自签证书<br/>RSA 2048, SAN IP"]
M["systemd/ipv6-proxy.service"] --> N["进程守护与开机自启"]
O["systemd/ndppd.conf.example"] --> P["NDPPD 静态邻居通告"]
Q["config.yaml.example"] --> R["可选外部化配置入口"]
```

**图表来源**
- [main.go:25-44](file://main.go#L25-L44)
- [main.go:32-37](file://main.go#L32-L37)
- [install.sh:73-91](file://scripts/install.sh#L73-L91)
- [gen_cert.sh:20-30](file://scripts/gen_cert.sh#L20-L30)
- [ipv6-proxy.service](file://systemd/ipv6-proxy.service)
- [ndppd.conf.example](file://systemd/ndppd.conf.example)
- [config.yaml.example](file://config.yaml.example)

章节来源
- [main.go:1-382](file://main.go#L1-L382)
- [install.sh:1-101](file://scripts/install.sh#L1-L101)
- [gen_cert.sh:1-38](file://scripts/gen_cert.sh#L1-L38)
- [README.md:1-98](file://README.md#L1-L98)

## 核心组件
- 主程序 main.go
  - 启动 HTTP CONNECT 与 SOCKS5 监听端口，解析命令行参数，初始化 IPv6 前缀与并发信号量。
  - **新增** 完善的多级日志系统，支持 INFO、WARN、ERROR、DEBUG 四个日志级别。
  - **新增** 调试模式支持，通过 `-v` 参数启用详细调试日志输出。
  - **增强** 错误报告机制，提供更详细的错误信息和上下文数据。
  - 随机源地址生成器基于给定前缀分配主机位，保证同一 /64 下多实例互不冲突。
  - 双向数据转发采用 io.Copy，连接建立后无状态中继。
- 安装脚本 install.sh
  - 自动安装依赖、下载源码或压缩包、编译、设置内核参数、安装 systemd 服务单元。
- 证书生成脚本 gen_cert.sh
  - 生成 RSA 2048 自签证书，支持 SAN IP，输出到指定目录，权限最小化。
- systemd 服务单元 ipv6-proxy.service
  - 定义服务运行用户、工作目录、重启策略、日志采集等。
- NDPPD 示例配置 ndppd.conf.example
  - 用于将本地 IPv6 前缀静态通告到上游路由器，使对端能正确路由到本机。
- 示例配置 config.yaml.example
  - 作为未来扩展的外部化配置模板（当前版本以命令行参数为主）。

**章节来源**
- [main.go:18-30](file://main.go#L18-L30)
- [main.go:32-44](file://main.go#L32-L44)
- [main.go:53-102](file://main.go#L53-L102)
- [main.go:134-231](file://main.go#L134-L231)
- [main.go:252-309](file://main.go#L252-L309)
- [install.sh:36-91](file://scripts/install.sh#L36-L91)
- [gen_cert.sh:10-38](file://scripts/gen_cert.sh#L10-L38)
- [ipv6-proxy.service](file://systemd/ipv6-proxy.service)
- [ndppd.conf.example](file://systemd/ndppd.conf.example)
- [config.yaml.example](file://config.yaml.example)

## 架构总览
下图展示了客户端通过 HTTP CONNECT 或 SOCKS5 访问后端目标时，IPv6 代理池的调用链路与关键控制点。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Proxy as "IPv6 代理(main.go)"
participant Logger as "日志系统"
participant Net as "网络栈/内核"
participant Target as "远端目标"
Note over Client,Proxy : HTTP CONNECT 流程
Client->>Proxy : "CONNECT host : port"
Proxy->>Logger : "[INFO] 记录请求信息"
Proxy->>Proxy : "校验方法/并发信号量"
Proxy->>Logger : "[DEBUG] 调试日志(可选)"
Proxy->>Net : "绑定随机 IPv6 源地址"
Proxy->>Target : "tcp6 连接目标"
Target-->>Proxy : "握手成功"
Proxy->>Logger : "[INFO] 连接成功日志"
Proxy-->>Client : "200 Connection established"
Proxy->>Target : "双向数据转发(io.Copy)"
Proxy-->>Client : "双向数据转发(io.Copy)"
Note over Client,Proxy : SOCKS5 流程
Client->>Proxy : "SOCKS5 握手+请求"
Proxy->>Logger : "[INFO] 记录握手信息"
Proxy->>Proxy : "解析目标(域名/IP)+端口"
Proxy->>Net : "绑定随机 IPv6 源地址"
Proxy->>Target : "tcp6 连接目标"
Target-->>Proxy : "握手成功"
Proxy->>Logger : "[INFO] 记录连接成功"
Proxy-->>Client : "回复成功"
Proxy->>Target : "双向数据转发(io.Copy)"
Proxy-->>Client : "双向数据转发(io.Copy)"
```

**图表来源**
- [main.go:25-44](file://main.go#L25-L44)
- [main.go:134-231](file://main.go#L134-L231)
- [main.go:252-309](file://main.go#L252-L309)

## 详细组件分析

### 主程序 main.go
- 启动与参数
  - 监听 HTTP CONNECT 与 SOCKS5 端口，默认分别位于 53420 与 53421；可通过 -http/-socks 调整。
  - -prefix 指定 IPv6 前缀（如 /112），用于随机源地址分配。
  - -c 限制最大并发连接数，防止资源耗尽。
  - **新增** -v 参数启用调试模式，输出详细的 DEBUG 级别日志。
- **新增** 日志系统架构
  - 初始化四个独立的日志记录器：infoLog、warnLog、errorLog、debugLog
  - INFO 日志：正常业务流程和重要事件
  - WARN 日志：警告信息和潜在问题
  - ERROR 日志：错误信息和异常处理
  - DEBUG 日志：仅在调试模式下输出的详细调试信息
- 随机源地址生成
  - 根据前缀掩码计算主机位长度，按字节填充随机值，确保在同一 /64 下不同实例互不冲突。
- HTTP CONNECT 处理
  - 仅允许 CONNECT 方法；若未携带端口则补全默认 443。
  - 使用 Hijack 接管连接，清空残留缓冲，设置 TCP_NODELAY。
  - 通过 net.Dialer 指定 LocalAddr 为随机 IPv6 源地址，超时 30s。
  - **增强** 详细的错误日志，包含目标地址、源IP、错误信息等完整上下文。
  - 成功后返回 200，随后双向拷贝数据。
- SOCKS5 处理
  - 握手阶段仅接受无认证模式。
  - 解析请求，支持域名与 IPv6 地址类型，拒绝 IPv4。
  - 同样通过随机 IPv6 源地址拨号，成功后返回 0x00 并转发数据。
  - **增强** 完整的错误处理链路，每个步骤都有相应的错误日志。
- 并发控制
  - 使用带缓冲 channel 作为信号量，超出限制直接拒绝，避免内存与文件描述符耗尽。
  - **新增** 并发限制的详细日志记录，便于监控和分析。

```mermaid
flowchart TD
Start(["进入 handleHTTP"]) --> LogInfo["[INFO] 记录请求信息"]
LogInfo --> CheckMethod{"是否 CONNECT?"}
CheckMethod --> |否| WarnLog["[WARN] 拒绝非 CONNECT 请求"]
WarnLog --> Err405["返回 405 Method Not Allowed"]
CheckMethod --> |是| ParseHost["解析 r.Host/r.URL.Host 并补全端口"]
ParseHost --> DebugLog["[DEBUG] 调试日志(可选)"]
DebugLog --> SemAcquire{"获取并发信号量"}
SemAcquire --> |失败| WarnLimit["[WARN] 连接数超限"]
WarnLimit --> Err503["返回 503 Too Many Connections"]
SemAcquire --> |成功| Hijack["Hijack 连接并清理缓冲"]
Hijack --> SetNoDelay["设置 TCP_NODELAY"]
SetNoDelay --> RandomIP["生成随机 IPv6 源地址"]
RandomIP --> DialTarget["tcp6 拨号目标(超时30s)"]
DialTarget --> DialOK{"拨号成功?"}
DialOK --> |否| ErrorLog["[ERROR] 记录详细错误信息"]
ErrorLog --> Err502["返回 502 Bad Gateway"]
DialOK --> |是| Send200["发送 200 Connection established"]
Send200 --> InfoOK["[INFO] 记录连接成功"]
InfoOK --> Relay["双向 io.Copy 转发"]
Relay --> End(["结束"])
```

**图表来源**
- [main.go:25-44](file://main.go#L25-L44)
- [main.go:134-231](file://main.go#L134-L231)

**章节来源**
- [main.go:18-30](file://main.go#L18-L30)
- [main.go:32-44](file://main.go#L32-L44)
- [main.go:53-102](file://main.go#L53-L102)
- [main.go:134-231](file://main.go#L134-L231)
- [main.go:252-309](file://main.go#L252-L309)

### 安装脚本 install.sh
- 功能概览
  - 检查 root 权限，安装 golang-go、git、ndppd、curl 等依赖。
  - 创建安装目录 /opt/ipv6-proxy 与配置目录 /etc/ipv6-proxy。
  - 从 GitHub 克隆或下载源码，执行 go mod tidy 与 go build 生成二进制。
  - 写入内核参数至 /etc/sysctl.d/99-ipv6-proxy.conf 并立即生效。
  - 复制 systemd 服务单元到 /etc/systemd/system/ipv6-proxy.service，重载 daemon。
- 注意事项
  - 需要公网 IPv6 前缀并已在本机添加 local 路由。
  - 需配置 ndppd 将前缀静态通告给上游路由器。
  - 建议在生产环境固定 Go 版本与构建产物校验。

**章节来源**
- [install.sh:1-101](file://scripts/install.sh#L1-L101)

### 证书生成脚本 gen_cert.sh
- 功能概览
  - 生成 RSA 2048 自签证书，CN 默认取本机外网 IPv4（可通过 ifconfig.me 获取），也可传入自定义 CN。
  - 输出 cert.pem 与 key.pem 到 /usr/local/etc/xray，key.pem 权限设为 600。
  - 提供验证命令输出 Subject 与 SAN 信息。
- 适用场景
  - 与 v2ray/xray 搭配使用时，作为 TLS 证书的快速生成工具。
- 安全建议
  - 生产环境建议使用受信任 CA 签发的证书；自签证书仅用于测试或内网隔离场景。

**章节来源**
- [gen_cert.sh:1-38](file://scripts/gen_cert.sh#L1-L38)

### systemd 服务管理
- 服务单元 ipv6-proxy.service
  - 由 install.sh 复制到 /etc/systemd/system/ipv6-proxy.service。
  - 典型职责包括：指定 ExecStart、Restart=always、WorkingDirectory、User/Group、StandardOutput/StandardError 等。
- 常用操作
  - 启用与启动：systemctl enable --now ipv6-proxy
  - 停止与禁用：systemctl stop/disable ipv6-proxy
  - 查看状态：systemctl status ipv6-proxy
  - 查看日志：journalctl -u ipv6-proxy -f
- 排障要点
  - 确认端口未被占用、IPv6 前缀已正确配置、内核参数已加载。
  - 关注日志中的 "[HTTP-FAIL]"、"[SOCKS5-FAIL]"、"[ERROR]" 等关键字。

**章节来源**
- [install.sh:87-91](file://scripts/install.sh#L87-L91)
- [ipv6-proxy.service](file://systemd/ipv6-proxy.service)

### NDPPD 配置
- 作用
  - 将本地 IPv6 前缀静态通告到上游路由器，使得对端设备可将该前缀路由到本机。
- 配置步骤
  - 参考示例配置文件，将规则中的接口与前缀替换为本机实际值。
  - 启用并重启 ndppd 服务。
- 验证
  - 使用 ip -6 route 查看是否存在指向本地的前缀路由。
  - 使用 ping6 或 curl 测试可达性。

**章节来源**
- [ndppd.conf.example](file://systemd/ndppd.conf.example)
- [README.md:59-77](file://README.md#L59-L77)

### 外部化配置
- 示例文件 config.yaml.example
  - 当前版本以命令行参数为主，示例文件可作为后续扩展的模板。
  - 建议在容器或编排平台中通过环境变量注入参数，保持镜像不可变。

**章节来源**
- [config.yaml.example](file://config.yaml.example)

## 依赖关系分析
- 运行时依赖
  - Go 标准库：net/http、net、crypto/rand、encoding/binary、sync、time、log、flag、io、errors、fmt、math/rand。
  - 操作系统能力：IPv6 路由、TCP 套接字、systemd 服务管理。
- 构建期依赖
  - Go 工具链与模块管理（go mod tidy/build）。
- 外部集成
  - ndppd：邻居发现代理，用于静态前缀通告。
  - v2ray/xray：可选，结合 smux 聚合缓解 conntrack 压力。

```mermaid
graph LR
Main["main.go"] --> Stdlib["Go 标准库"]
Main --> OS["Linux 内核/网络栈"]
Main --> Logger["多级日志系统"]
Install["install.sh"] --> Apt["apt 包管理器"]
Install --> Systemd["systemd"]
Cert["gen_cert.sh"] --> OpenSSL["OpenSSL"]
Service["ipv6-proxy.service"] --> Systemd
NDPPD["ndppd.conf.example"] --> Kernel["IPv6 邻居表"]
```

**图表来源**
- [main.go:1-16](file://main.go#L1-L16)
- [main.go:25-44](file://main.go#L25-L44)
- [install.sh:36-91](file://scripts/install.sh#L36-L91)
- [gen_cert.sh:20-30](file://scripts/gen_cert.sh#L20-L30)
- [ipv6-proxy.service](file://systemd/ipv6-proxy.service)
- [ndppd.conf.example](file://systemd/ndppd.conf.example)

**章节来源**
- [main.go:1-16](file://main.go#L1-L16)
- [main.go:25-44](file://main.go#L25-L44)
- [install.sh:36-91](file://scripts/install.sh#L36-L91)
- [gen_cert.sh:20-30](file://scripts/gen_cert.sh#L20-L30)
- [ipv6-proxy.service](file://systemd/ipv6-proxy.service)
- [ndppd.conf.example](file://systemd/ndppd.conf.example)

## 性能与容量规划
- 并发与限流
  - 通过 -c 控制最大并发连接数，建议根据 CPU 核数、内存与目标带宽评估。
  - 观察日志中 "[WARN] 连接数超限" 频率，适当提高 -c 或扩容实例。
- 内核参数
  - 启用 net.ipv6.ip_nonlocal_bind、net.ipv6.conf.all.forwarding。
  - 调整 IPv6 邻居表阈值 gc_thresh1/2/3，避免在高并发下丢包。
  - 开启 net.ipv4.tcp_tw_reuse，缩短 tcp_fin_timeout，扩大 ip_local_port_range。
- 连接复用
  - 与 v2ray/xray 配合使用 smux，减少短连接数量，降低 conntrack 压力。
- 资源监控
  - 监控指标：CPU、内存、FD 使用数、连接数、错误率、延迟分位数。
  - 推荐接入 Prometheus + Grafana，暴露进程级指标或通过 sidecar 采集。

## 故障排查指南
- **新增** 日志系统使用
  - **调试模式**：使用 `-v` 参数启动程序，启用详细的 DEBUG 级别日志输出
  - **日志级别**：
    - `[INFO]`：正常业务流程，如连接建立、请求处理
    - `[WARN]`：警告信息，如方法不支持、连接数超限
    - `[ERROR]`：错误信息，如连接失败、Hijack 失败
    - `[DEBUG]`：详细调试信息，仅在 `-v` 模式下输出
  - **日志过滤**：使用 `journalctl -u ipv6-proxy -f --grep "\[ERROR\]"` 过滤错误日志
- 无法绑定端口
  - 现象：启动时报端口占用或权限不足。
  - 排查：确认端口未被占用、非特权端口无需 root；必要时使用 iptables/nftables 放行。
  - **日志定位**：查看 `[ERROR]` 级别的启动失败日志
- IPv6 前缀不可达
  - 现象：拨号失败或目标不可达。
  - 排查：检查本机 local 路由是否正确、ndppd 是否运行且规则匹配、上游路由器是否学习到前缀。
  - **日志定位**：关注 `[HTTP-FAIL]` 和 `[SOCKS5-FAIL]` 日志，查看具体的错误信息
- 并发受限频繁拒绝
  - 现象：大量 "[WARN] 连接数超限" 日志。
  - 排查：提升 -c 或横向扩容实例；检查下游目标响应时间与错误率。
  - **调试模式**：使用 `-v` 参数查看详细的并发控制日志
- 证书问题
  - 现象：TLS 握手失败或 SAN 不匹配。
  - 排查：使用 gen_cert.sh 重新生成，确认 CN/SAN 与实际域名或 IP 一致；生产环境改用受信任 CA。

**章节来源**
- [main.go:25-44](file://main.go#L25-L44)
- [main.go:134-231](file://main.go#L134-L231)
- [main.go:252-309](file://main.go#L252-L309)
- [main.go:196-202](file://main.go#L196-L202)
- [main.go:283-287](file://main.go#L283-L287)
- [install.sh:73-85](file://scripts/install.sh#L73-L85)
- [gen_cert.sh:20-38](file://scripts/gen_cert.sh#L20-L38)

## 结论
本项目以极简架构实现了高可用的 IPv6 出口代理池，具备双协议支持与并发限流能力。**新增的完善日志系统和调试模式**显著提升了故障排查能力和运维效率。通过一键安装脚本与 systemd 服务管理，可在物理机、虚拟机与容器中快速部署。生产环境应重点关注 IPv6 路由与邻居通告、内核参数调优、证书管理与监控告警，并结合 smux 等技术提升整体吞吐与稳定性。

## 附录：常用命令与最佳实践

- 快速开始
  - 一键安装：参考 README 提供的 curl 管道方式执行 install.sh。
  - 手动编译运行：参考 README 的手动部署步骤。
  - **新增** 调试模式：`./ipv6-proxy -v -http 0.0.0.0:53420 -socks 0.0.0.0:53421 -prefix 240e:6b0:50::/112`
- 服务管理
  - systemctl enable --now ipv6-proxy
  - systemctl restart ipv6-proxy
  - journalctl -u ipv6-proxy -f
  - **新增** 日志过滤：`journalctl -u ipv6-proxy -f --grep "\[ERROR\]"`
- **新增** 日志管理最佳实践
  - 生产环境默认关闭调试模式，仅保留 INFO、WARN、ERROR 级别
  - 故障排查时临时启用 `-v` 参数获取详细调试信息
  - 定期轮转日志文件，避免磁盘空间耗尽
  - 集中收集日志到 ELK Stack 或类似日志分析平台
- 安全加固
  - 最小权限原则：以非 root 用户运行服务，仅开放必要端口。
  - 证书管理：生产环境使用受信任 CA；定期轮换证书与私钥。
  - 访问控制：结合防火墙与 ACL 限制来源 IP 段。
- 性能监控
  - 指标采集：进程 FD、goroutine 数、连接数、错误率、P95/P99 延迟。
  - 告警策略：错误率突增、连接数饱和、内存/CPU 异常波动。
  - **新增** 日志监控：监控 ERROR 级别日志数量和增长趋势
- 故障恢复
  - 自动重启：systemd Restart=always。
  - 健康检查：对外暴露轻量健康端点（可扩展），供负载均衡探针探测。
  - 灰度发布：先小流量验证，再逐步放量。

**章节来源**
- [README.md:13-25](file://README.md#L13-L25)
- [README.md:26-98](file://README.md#L26-98)
- [install.sh:92-101](file://scripts/install.sh#L92-L101)
- [main.go:23](file://main.go#L23)
- [main.go:39-44](file://main.go#L39-L44)